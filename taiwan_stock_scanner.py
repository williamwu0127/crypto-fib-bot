...
# 優化 1: 避免將 Webhook 直接寫死在公開程式碼中。
# 預設會優先讀取系統的 "DISCORD_WEBHOOK" 環境變數，若無則使用後方的字串。
# 測試時，請將下方字串替換為你真實的 Webhook URL。
WEBHOOK_URL = os.getenv("DISCORD_WEBHOOK", "https://discord.com/api/webhooks/1543491812101062697/qM1ZaG4UGxu5zoyWxWZJVeL3SLDNCcKTGobB4OhBYRAazuSHRz-WHn2mLSvJ9RwKgxgf")
...
def send_msg(payload):
    if WEBHOOK_URL == "https://discord.com/api/webhooks/1543491812101062697/qM1ZaG4UGxu5zoyWxWZJVeL3SLDNCcKTGobB4OhBYRAazuSHRz-WHn2mLSvJ9RwKgxgf":
        print("尚未設定 Webhook URL，跳過 Discord 發送。")
        return
...
def translate_to_zh(text):
    """優化 2: 強化翻譯 API 的 User-Agent 與加入錯誤日誌"""
    if not text:
        return ""
    try:
        ...
        # 使用更真實的瀏覽器標頭，避免被 Google API 阻擋 (403 錯誤)
        headers = {
    ...
    return text # 若發生錯誤，退回顯示英文原文

def get_market_analysis(args):
    """優化 3: 加入 Try-Except 印出錯誤，避免靜默失敗"""
